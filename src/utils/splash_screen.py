from utils import config


class SplashScreen:

    def __init__(self):
        # Nome(s) do(s) criador(es)
        self.created_by = "Cleverton Liltk, Gustavo Christ, Jhony Souza, Lucio Ewald e Wellington da Silva"
        self.professor = "Prof. M.Sc. Howard Roatti"
        self.disciplina = "Banco de Dados"
        self.semestre = "2022/2"

    def get_documents_count(self, collection_name):
        # Retorna o total de registros computado pela query
        df = config.query_count(collection_name=collection_name)
        return df[f"total_{collection_name}"].values[0]

    def get_updated_screen(self):
        print(f"""
+------------------------------------------------------------------------------------------+
|  # SISTEMA DE CONTROLE DE FATURAS:
|
|  TOTAL DE REGISTROS:
|  1 - Clientes: {str(self.get_documents_count(collection_name="clientes")).rjust(5)}
|  2 - Faturas: {str(self.get_documents_count(collection_name="fatura")).rjust(5)}
|
|  DESENVOLVIDO POR:
|  {self.created_by}
|
|  PROFESSOR: {self.professor}
|
|  DISCIPLINA: {self.disciplina} | {self.semestre}
+------------------------------------------------------------------------------------------+
""")
